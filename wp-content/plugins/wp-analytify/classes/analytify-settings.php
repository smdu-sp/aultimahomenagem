			...
			$settings_fields = array(
				'wp-analytify-authentication' => array(
					array(
						...
						'desc'    => wp_sprintf( '<p class="description">%s <code>%s</code><br /> %s <code>%s</code>%s </p>', __( 'Manually add UA Tracking ID that looks a like', 'wp-analytify' ), 'UA-XXXXXXXX-XX', __( 'Our default tracking method is newly recommended', 'wp-analytify' ), __( 'Global Site Tag (gtag.js)', 'wp-analytify' ), __( 'by Google Analytics.', 'wp-analytify' ) ),
						'type'    => 'text',
						'default' => '',
					),
				),
		  ...
		  if ( empty( $current_values ) ) {
		    $html .= '';
		      ...
		      foreach( $current_values as $current_value => $vals ) {
		        ...
		        $html .= '</select></td>';
		        $html .= sprintf( '<td><input type="number" class="dimension-id" name="%1$s[%2$s]['.$count.'][id]" id="%1$s[%2$s]" value="'.$vals['id'].'"></td>', $args['section'], $args['id'] );
